 ...
 *
 * @param searchValue 検索クエリ文字列
 * @param limit 返す候補数（デフォルト: 5）
 * @returns サジェスト候補配列（前方一致を優先、次に部分一致）
 */
export function getSuggestions(searchValue: string, limit = 5): string[] {
  if (isEmptyQuery(searchValue)) {
    return [];
  }
  ...
  // 転置インデックスのキー（トークン）から前方一致と部分一致を検索
  for (const token of Object.keys(typedSearchIndex)) {
    if (token.startsWith(queryLower)) {
      prefixMatches.push(token);
    } else if (token.includes(queryLower)) {
      partialMatches.push(token);
    }

    // 制限数に達したら終了
    if (prefixMatches.length + partialMatches.length >= limit * 2) {
      break;
    }
  }
...
export function getSearchStats() {
  return {
    totalTokens: Object.keys(typedSearchIndex).length,
    totalArticles: typedSearchData.length,
    averageTokensPerArticle:
      typedSearchData.length === 0
        ? 0
        : typedSearchData.reduce((sum, item) => sum + item.tokens.length, 0) / typedSearchData.length,
  };
}
