...
プルリクエストのタイトルと説明を生成する際、以下の形式と内容に従うこと。良いPRの説明はレビュープロセスを加速し、変更の意図と影響を明確に伝える。
...
変更の目的と背景を1-2文で説明する。「何を」と「なぜ」を明確にする。
...
TF-IDF計算処理の最適化により、記事類似度計算の処理時間を約50%短縮した。大量の記事を処理する場合のビルド時間を短縮するための改善である。
